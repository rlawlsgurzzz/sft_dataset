# Coverage Summary

## intent_family

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| attack | 0.0% | 24% | 0 |
| move | 0.0% | 22% | 0 |
| skill | 0.0% | 26% | 0 |
| skillControl | 0.0% | 8% | 0 |
| wait | 0.0% | 8% | 0 |
| empty | 0.0% | 12% | 0 |

## Actor Selection

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| explicit_actor | 0.0% | 40% | 0 |
| explicit_multi_actor | 0.0% | 12% | 0 |
| global_condition | 0.0% | 16% | 0 |
| global_role_based | 0.0% | 12% | 0 |
| global_state_based | 0.0% | 15% | 0 |
| no_valid_actor | 0.0% | 5% | 0 |

## Target Selection

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| explicit_enemy_target | 0.0% | 21% | 0 |
| explicit_ally_target | 0.0% | 13% | 0 |
| nearest_enemy | 0.0% | 8% | 0 |
| lowest_hp_enemy | 0.0% | 8% | 0 |
| highest_threat_enemy | 0.0% | 7% | 0 |
| role_based_enemy | 0.0% | 8% | 0 |
| pressure_source_enemy | 0.0% | 6% | 0 |
| safe_ally | 0.0% | 6% | 0 |
| low_hp_ally | 0.0% | 6% | 0 |
| backline_ally | 0.0% | 4% | 0 |
| invalid_explicit_target | 0.0% | 7% | 0 |
| none | 0.0% | 6% | 0 |

## Action Pattern

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| attack_only | 0.0% | 13% | 0 |
| move_only | 0.0% | 13% | 0 |
| move_then_attack | 0.0% | 11% | 0 |
| skill_only | 0.0% | 19% | 0 |
| move_then_skill | 0.0% | 4% | 0 |
| wait_only | 0.0% | 6% | 0 |
| wait_then_attack | 0.0% | 3% | 0 |
| wait_then_skill | 0.0% | 2% | 0 |
| skillControl_defer | 0.0% | 4% | 0 |
| skillControl_forbid | 0.0% | 4% | 0 |
| multi_actor_same_target | 0.0% | 6% | 0 |
| multi_actor_different_targets | 0.0% | 3% | 0 |
| empty_action_expected | 0.0% | 12% | 0 |

## Command Style

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| direct_korean | 0.0% | 35% | 0 |
| casual_korean | 0.0% | 20% | 0 |
| elliptical_korean | 0.0% | 20% | 0 |
| tactical_korean | 0.0% | 15% | 0 |
| rough_korean | 0.0% | 10% | 0 |

## Skill Family

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| enemy_single_target_attack | 0.0% | 18% | 0 |
| self_buff | 0.0% | 14% | 0 |
| ally_shield | 0.0% | 13% | 0 |
| ally_heal | 0.0% | 11% | 0 |
| ally_resurrection | 0.0% | 13% | 0 |
| enemy_aoe_attack | 0.0% | 12% | 0 |
| enemy_debuff | 0.0% | 8% | 0 |
| mobility_skill | 0.0% | 5% | 0 |
| no_skill | 0.0% | 6% | 0 |

## Skill Target Kind

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| enemy_alive | 0.0% | 36% | 0 |
| ally_alive | 0.0% | 22% | 0 |
| self | 0.0% | 16% | 0 |
| ally_dead | 0.0% | 12% | 0 |
| enemy_dead | 0.0% | 5% | 0 |
| none | 0.0% | 9% | 0 |

## Conflict Type

| key | current_ratio | target_ratio | count |
|---|---:|---:|---:|
| null | 0.0% | 48% | 0 |
| text_enemy_target_but_self_skill | 0.0% | 8% | 0 |
| text_enemy_target_but_ally_skill | 0.0% | 9% | 0 |
| text_ally_target_but_enemy_skill | 0.0% | 9% | 0 |
| text_dead_target_but_skill_cannot_target_dead | 0.0% | 8% | 0 |
| text_living_target_but_resurrection_skill | 0.0% | 6% | 0 |
| skill_actor_has_no_skill | 0.0% | 7% | 0 |
| aoe_single_center_required | 0.0% | 5% | 0 |

## Taxonomy Errors

No taxonomy errors found.
