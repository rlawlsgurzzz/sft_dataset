...
학습 대상은 단순한 문장 paraphrase가 아니다.  
명령 의미, actor/target/action 판단, skill target side, 현재 전장 상태, 런타임 유효성, dialog/action 일치까지 포함한 전투 명령 파서 동작 전체다.
...
SFT label은 처음부터 유효한 assistant output이어야 한다.  
sanitizer가 보정한 결과는 label로 사용하지 않는다.
...
teacher는 `source_ref`를 생성하지 않는다.  
teacher는 `validator_result`를 생성하지 않는다.

teacher는 payload에 포함된 다음 정보를 기준으로 샘플을 만든다.
...
검증을 통과한 sample은 accepted에 저장된다.  
이때 validator가 `area_situation`을 읽어 `input.commandAnalysis`를 계산해 추가한다.

검증을 통과하지 못한 sample은 rejected에 저장된다.
...
학생 SLM은 `commandAnalysis`를 참고하지만, 명령의 actor/target/action 의미 해석은 직접 수행한다.  
`commandAnalysis`는 명령 의도 파서가 아니라 런타임 제약 요약이다.
...
raw sample은 teacher LLM의 원본 생성물이다.  
raw sample은 아직 학습 데이터가 아니다.
...
coverage report는 accepted 샘플을 기준으로 재생성된다.
...
`canSkillTargetDead=true`는 skill target에서만 적용되는 특수 예외다.  
일반 attack, move, actor 선정에는 적용되지 않는다.
...
move는 좌표를 출력하지 않는다.  
엔진이 `subtype`, `movementType`, `to`를 바탕으로 실제 이동 위치를 계산한다.
...
skill target은 명령문에 적힌 대상만으로 결정하지 않는다.  
actor의 skill field와 현재 전장 상태를 함께 기준으로 결정한다.
...
dialog는 schema 안정성과 actor 일치가 우선이다.  
표현 다양성은 부차적이다.
...
attack
  explicit_actor
    explicit_enemy_target
      attack_only
        simple_clear_target
          "A_01, E_02를 공격해." @ 1 @ [] @ accepted_20260512_005252.jsonl_seed_master_0001
```

여기서 command slot은 특정 general path 아래의 `base_command_text + edge_flags` 조합으로 본다.
...
`gold`는 정답 의미를 검증하기 위한 보조 기준이다.  
학생 모델의 최종 출력에 `gold`는 포함되지 않는다.

---

## 13. split과 command_text 표현 pool
...
```text
1. 요청 split의 기존 accepted 표현은 existing_valid_paraphrase_samples로 전달된다.
2. 다른 split의 기존 accepted 표현은 other_split_reserved_command_texts로 전달된다.
...
6. 요청 split의 표현 pool이 가득 찬 뒤에는 같은 split pool 안에서만 command_text를 순환 재사용할 수 있다.
7. 같은 요청에서 새로 만든 unique command_text도 이후 cycle source가 될 수 있다.
8. 다른 split의 reserved command_text는 cycle source로 사용할 수 없다.
9. command_text를 재사용하더라도 area_situation, gold, output은 새로 구성한다.
```

즉, split 간 표현 pool은 섞지 않는다.  
하지만 다른 split 표현은 duplicate 금지용 reference로 사용한다.

### 13.1 payload 의미

예를 들어 validation 요청이면 payload의 split 관련 부분은 다음과 같은 형태가 된다.
...
```json
{
  ...
  "existing_valid_paraphrase_samples": [
    {
      "command_text": "validation split에 이미 accepted된 표현",
      "command_style": "direct_korean"
    }
  ],
  "other_split_reserved_command_texts": [
    {
      "split": "train",
      "command_text": "train에 이미 accepted된 표현",
      "command_style": "direct_korean"
    },
    {
      "split": "test",
      "command_text": "test에 이미 accepted된 표현",
      "command_style": "direct_korean"
    }
  ],
  "command_text_policy": {
    "target_split": "validation",
    "same_split_expression_pool_size": 4,
    "existing_same_split_expression_count": 1,
    "other_split_reserved_expression_count": 2,
    "new_unique_command_texts_to_create": 3,
    "samples_using_same_split_cycle": 7
  }
}
```
...
```text
validation pool 목표치 = 4
validation 기존 표현 = 1
새 표현 3개 생성
나머지 7개는 validation pool 안에서 cycle
train/test 표현은 duplicate 금지용으로만 사용
```

요청 split이 validation이면 validation 표현만 cycle source가 된다.  
train/test 표현은 새 command_text와 exact duplicate가 되지 않도록 확인하는 reserved 목록이다.

### 13.2 command slot 탐색과 split pool

generation request는 accepted 전체를 읽어서 요청 path의 command slot을 찾는다.  
이때 command slot 탐색은 split과 무관하게 accepted 전체를 기준으로 한다.

하지만 teacher에게 전달되는 표현 pool은 split 기준으로 나뉜다.

```text
target_split과 같은 accepted:
  existing_valid_paraphrase_samples
  cycle 가능

target_split과 다른 accepted:
  other_split_reserved_command_texts
  duplicate 금지용
  cycle 불가
```

따라서 모든 split에 accepted sample이 하나씩 있을 필요는 없다.  
해당 command slot이 accepted 전체에 하나라도 있으면 요청은 가능하다.

요청 split에 기존 표현이 없으면 teacher는 해당 split의 pool limit까지 새 command_text를 만든다.  
요청 개수가 pool limit을 초과하면 같은 요청에서 새로 만든 command_text를 포함하여 같은 split pool 안에서 cycle한다.

---

## 14. 주요 파일

### 14.1 `config/taxonomy_sot.json`
...
`taxonomy_sot.json`이 분류 기준의 machine-readable source of truth다.

### 14.2 `scripts/sft_taxonomy.py`
...
### 14.3 `scripts/sft_coverage_report.py`
...
`general_coverage.md`는 general taxonomy 기준으로 command slot별 accepted 수를 보여준다.  
`skill_coverage.md`는 skill taxonomy 기준으로 skill case별 accepted 수를 보여준다.

### 14.4 `scripts/sft_generation_request.py`
...
### 14.5 `scripts/sft_teacher_client.py`
...
4. target split을 지정해 request 또는 generate를 실행한다.
5. sft_generation_request.py가 teacher payload를 만든다.
6. payload에는 selected bucket, split pool, reserved command_text, generation contract가 포함된다.
7. sft_teacher_client.py가 teacher LLM을 호출해 raw sample 후보를 저장한다.
8. sft_validator.py가 raw sample을 검증한다.
9. 통과 sample은 commandAnalysis가 추가되어 accepted에 저장된다.
10. 실패 sample은 rejected에 저장된다.
11. coverage report를 다시 생성한다.
12. accepted sample을 SFT 학습 데이터로 변환한다.
```
