...
# 30초 이내 API 거절은 수락될 때까지 재시도하고, 30초 이상 걸린 거절은 해당 요청을 스킵한다.
...
DEFAULT_API_RETRY_INITIAL_MIN_SEC = 5.0
DEFAULT_API_RETRY_INITIAL_MAX_SEC = 7.0
DEFAULT_API_RETRY_MIN_INCREMENT_SEC = 1.0
DEFAULT_API_RETRY_MAX_INCREMENT_SEC = 2.0
DEFAULT_API_REJECTION_THRESHOLD_SEC = 30.0
...
class LongApiRejectionError(RuntimeError):
    def __init__(self, message: str, elapsed_sec: float) -> None:
        super().__init__(message)
        self.elapsed_sec = elapsed_sec


def run_teacher_until_accepted_or_long_rejected(
    *,
    input_path: Path,
    output_path: Path,
    trace_path: Path,
    ...
    max_tokens: int,
    stream_output: bool,
    initial_delay_min_sec: float,
    initial_delay_max_sec: float,
    delay_min_increment_sec: float,
    delay_max_increment_sec: float,
    rejection_threshold_sec: float,
) -> dict[str, Any]:
    if initial_delay_min_sec < 0:
        raise ValueError("initial_delay_min_sec must be at least zero")
    if initial_delay_max_sec < initial_delay_min_sec:
        raise ValueError("initial retry delay range is invalid")
    if delay_min_increment_sec < 0 or delay_max_increment_sec < 0:
        raise ValueError("retry delay increments must be at least zero")
    if rejection_threshold_sec <= 0:
        raise ValueError("rejection_threshold_sec must be greater than zero")
    ...
    while True:
        ...
        try:
            ...
            if elapsed_sec >= rejection_threshold_sec:
                raise LongApiRejectionError(
                    (
                        f"api rejection returned after {elapsed_sec:.2f} sec "
                        f"(threshold={rejection_threshold_sec:.2f} sec): {error}"
                    ),
                    elapsed_sec=elapsed_sec,
                ) from error

            failed_count += 1
            delay_min = initial_delay_min_sec + delay_min_increment_sec * (failed_count - 1)
            delay_max = initial_delay_max_sec + delay_max_increment_sec * (failed_count - 1)
            delay_sec = random.uniform(delay_min, delay_max)

            print(
                "api_request_failed_fast: "
                f"failed_count={failed_count}, "
                f"request_elapsed_sec={elapsed_sec:.2f}, "
...
def run_auto_generate(args: argparse.Namespace) -> int:
    ...
    try:
        for task in selected_tasks:
            ...
            try:
                teacher_result = run_teacher_until_accepted_or_long_rejected(
                    ...
                    initial_delay_min_sec=args.api_retry_initial_min_sec,
                    initial_delay_max_sec=args.api_retry_initial_max_sec,
                    delay_min_increment_sec=args.api_retry_min_increment_sec,
                    delay_max_increment_sec=args.api_retry_max_increment_sec,
                    rejection_threshold_sec=args.api_rejection_threshold_sec,
                )
            except LongApiRejectionError as error:
                task_finished_at = datetime.now().isoformat(timespec="seconds")
                elapsed_sec = round(time.monotonic() - task_start_monotonic, 2)
                error_record = {
                    "time": task_finished_at,
                    ...
                    "error_type": type(error).__name__,
                    "error_message": str(error),
                    "api_rejection_elapsed_sec": round(error.elapsed_sec, 2),
                    "api_rejection_threshold_sec": args.api_rejection_threshold_sec,
                }
                append_jsonl(error_path, error_record)
                ...
                records.append(
                    {
                        ...
                        "trace_file": relpath(trace_output_path, dataset_root),
                        "validate_result": {
                            "api_long_rejection_skip": str(error),
                            "api_rejection_elapsed_sec": round(error.elapsed_sec, 2),
                        },
                        "accepted_count": 0,
                        "rejected_count": 0,
                        "status": "api_long_rejection_skip",
                        "started_at": task_started_at,
                        "finished_at": task_finished_at,
                        "elapsed_sec": elapsed_sec,
                    }
                ...
                print(f"api_long_rejection_skip: {error}")
            ...
            if teacher_result is not None:
                try:
                    ...
                    if args.stop_on_rejected and rejected_count > 0:
                        status = "stopped_on_rejected"
                        break
    ...
    finished_at = datetime.now().isoformat(timespec="seconds")
    error_statuses = {"api_long_rejection_skip", "request_build_error", "validation_error"}
    final_status = (
        "completed_with_skips_or_errors"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run sequential SFT teacher generation batches with compromised API retry policy."
    )
    ...
    parser.add_argument("--api-retry-max-increment-sec", type=float, default=DEFAULT_API_RETRY_MAX_INCREMENT_SEC)
    parser.add_argument("--api-rejection-threshold-sec", type=float, default=DEFAULT_API_REJECTION_THRESHOLD_SEC)
    return parser
...
def main() -> None:
    ...
    if args.api_retry_initial_min_sec < 0:
        parser.error("--api-retry-initial-min-sec must be at least zero")
    if args.api_retry_initial_max_sec < args.api_retry_initial_min_sec:
        parser.error("--api-retry-initial-max-sec must be greater than or equal to --api-retry-initial-min-sec")
    if args.api_retry_min_increment_sec < 0:
        parser.error("--api-retry-min-increment-sec must be at least zero")
    if args.api_retry_max_increment_sec < 0:
        parser.error("--api-retry-max-increment-sec must be at least zero")
    if args.api_rejection_threshold_sec <= 0:
        parser.error("--api-rejection-threshold-sec must be greater than zero")
